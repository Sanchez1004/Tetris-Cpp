...
void UserInterface::DrawScoreComponent() {
	...
	const Vector2 textSize = MeasureTextEx(font, scoreText, game->scoreFontSize, 2);
	...
	if (game->IsGameOver()) {
		...
		*/
		const Vector2 totalScoreMessageSize = MeasureTextEx(font, totalScoreMessage.c_str(), game->gameOverFontSize, 0.5);
